...
# Requires: rsync jq sshpass

# declare script and data locations
CONFIG=test/scripts/config.json

# sleep to allow Redis to cache ALL info, replace with logic to check linked-list lengths
sleep 5
# Interval for time(x), crontab executes sia-host-info.sh hourly by default
XINTERVALS=96
